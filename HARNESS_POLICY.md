...
The PR lane runs `pnpm qa:harness-v2` and produces versioned JSONL traces plus schema-validated environment-capability, validation, architecture, changed-coverage, changed-mutation, and deterministic hash receipts under `destiny-product/qa/artifacts/harness/`. The complete gate requires a responsive Docker- or Podman-compatible engine before starting its Supabase lane and fails early with a capability receipt when infrastructure is absent. Network access fails closed unless a declared mode permits it. Application telemetry uses correlation IDs and versioned structured events with recursive secret and PII redaction.
