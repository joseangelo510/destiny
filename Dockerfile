...
RUN test "$GIT_SHA" = "ed8c29aff96f8b4a2644b3806077ceb6863fd72b" \
 && test "$RELEASE_TAG" = "rebound-seo-v1.1.1" \
...
USER app
EXPOSE 3000
CMD ["sh","-c","SHA=\"$(cat .next/static/build-sha.txt)\" && TAG=\"$(cat .next/static/build-tag.txt)\" && BUILD_ENV=\"$(cat .next/static/build-env.txt)\" && BUILD_SITE_URL=\"$(cat .next/static/build-site-url.txt)\" && test \"$SHA\" = \"ed8c29aff96f8b4a2644b3806077ceb6863fd72b\" && test \"$TAG\" = \"rebound-seo-v1.1.1\" && test \"$BUILD_ENV\" = \"production\" && test \"$BUILD_SITE_URL\" = \"https://app.reboundseo.com\" && test \"$SHA\" = \"$DESTINY_BUILD_SHA\" && echo \"DESTINY_BUILD_SHA=$SHA\" && echo \"DESTINY_BUILD_TAG=$TAG\" && echo \"DESTINY_BUILD_ENV=$BUILD_ENV\" && echo \"DESTINY_BUILD_SITE_URL=$BUILD_SITE_URL\" && echo \"FLY_IMAGE_REF=${FLY_IMAGE_REF:-unset}\" && exec node_modules/.bin/next start -H 0.0.0.0 -p ${PORT:-3000}"]
