...
RUN test "$GIT_SHA" = "1bf6309849978de264912e6a640e44b50b5f3ada" \
 && test "$RELEASE_TAG" = "rebound-seo-v1.1.12" \
...
USER app
EXPOSE 3000
CMD ["sh","-c","SHA=\"$(cat .next/static/build-sha.txt)\" && TAG=\"$(cat .next/static/build-tag.txt)\" && BUILD_ENV=\"$(cat .next/static/build-env.txt)\" && BUILD_SITE_URL=\"$(cat .next/static/build-site-url.txt)\" && test \"$SHA\" = \"1bf6309849978de264912e6a640e44b50b5f3ada\" && test \"$TAG\" = \"rebound-seo-v1.1.12\" && test \"$BUILD_ENV\" = \"production\" && test \"$BUILD_SITE_URL\" = \"https://app.reboundseo.com\" && test \"$SHA\" = \"$DESTINY_BUILD_SHA\" && echo \"DESTINY_BUILD_SHA=$SHA\" && echo \"DESTINY_BUILD_TAG=$TAG\" && echo \"DESTINY_BUILD_ENV=$BUILD_ENV\" && echo \"DESTINY_BUILD_SITE_URL=$BUILD_SITE_URL\" && echo \"FLY_IMAGE_REF=${FLY_IMAGE_REF:-unset}\" && exec node_modules/.bin/next start -H 0.0.0.0 -p ${PORT:-3000}"]
