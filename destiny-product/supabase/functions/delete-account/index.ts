...
export default { fetch: withSupabase({ auth: "user" }, async (request, context) => {
  ...
  const { data: ownedOrganizations, error: organizationError } = await context.supabaseAdmin.from("organizations").select("id").eq("owner_id", userId);
  ...
  if (organizationIds.length > 0) {
    const { data: otherMembers, error: memberError } = await context.supabaseAdmin.from("organization_members").select("organization_id,user_id").in("organization_id", organizationIds).neq("user_id", userId).limit(1);
    if (memberError) return json({ error: "Destiny could not verify workspace ownership." }, 500);
    if ((otherMembers ?? []).length > 0) return json({ error: "Transfer ownership of shared workspaces before deleting your account." }, 409);
    const { error: deleteOrganizationsError } = await context.supabaseAdmin.from("organizations").delete().in("id", organizationIds);
