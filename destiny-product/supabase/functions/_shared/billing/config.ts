...
export type BillingConfig = { key: string; accountId: string; webhookSecret: string; priceIds: StripePrices; origin: string; livemode: boolean };
export function billingConfig(env: (name: string) => string | undefined): BillingConfig | null {
  ...
  return { key, accountId, webhookSecret, priceIds, origin, livemode: mode === "live" };
}
