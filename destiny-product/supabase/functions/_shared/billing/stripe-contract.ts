...
export function checkoutParameters(input: { ownerId: string; customerId: string; plan: PlanId; trialEligible: boolean; origin: string; priceIds: StripePrices }): Stripe.Checkout.SessionCreateParams {
  ...
  return {
    metadata: { rebound_trial: String(input.trialEligible) },
...
export function validateSubscription(subscription: Stripe.Subscription, expected: { customerId: string; livemode: boolean; priceIds: StripePrices }): SubscriptionState {
  ...
  // Paid upgrades may be prorated. Credits, trial invoices outside the current
  // item period, and invoices for another subscription cannot establish access.
  const invoiceCustomer = typeof invoice?.customer === "string" ? invoice.customer : invoice?.customer?.id;
  const invoiceSubscription = invoice?.parent?.subscription_details?.subscription;
  const invoiceSubscriptionId = typeof invoiceSubscription === "string" ? invoiceSubscription : invoiceSubscription?.id;
  const line = invoice?.status === "paid" && invoiceCustomer === customer && invoiceSubscriptionId === subscription.id
    ? invoice.lines.data.find(line => line.parent?.subscription_item_details?.subscription_item === item.id
      && line.pricing?.price_details?.price === item.price.id && line.amount >= 0
      && line.period.start >= item.current_period_start && line.period.start < line.period.end
...
export async function verifyStripeEventAsync(rawBody: string, signature: string, secret: string, livemode: boolean): Promise<Stripe.Event> {
  if (!signature || !secret) throw new Error("Stripe signature configuration missing");
  const stripe = new Stripe("sk_unused_signature_verification");
  const event = await stripe.webhooks.constructEventAsync(rawBody, signature, secret, 300, Stripe.createSubtleCryptoProvider());
  if (event.livemode !== livemode || event.account) throw new Error("Stripe event account or mode mismatch");
  return event;
}
