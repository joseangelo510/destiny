...
import { authEmailVerification } from "./verified-user.ts";
export async function reserveTransactionalEmail(admin: SupabaseClient, actorId: string, websiteId: string, kind: "welcome" | "progress", requestKey: string, recipient: string, env: (name: string) => string | undefined) {
  ...
  try {
    const verification = await authEmailVerification(admin, actorId);
    if (verification === "unavailable") return { allowed: false, reason: "unavailable" };
    if (verification !== "verified") return { allowed: false, reason: "verification_required" };
    const { data, error } = await admin.rpc("reserve_transactional_email_v2", { p_actor_id: actorId, p_website_id: websiteId, p_kind: kind, p_request_key: requestKey, p_actor_verified: true });
