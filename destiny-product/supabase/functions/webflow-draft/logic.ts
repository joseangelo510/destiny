...
export function planWebflowFieldData(input: {
  ...
  for (const field of normalizeFields(input.fields)) {
    ...
    if ((field.type === "PlainText" || field.type === "TextArea") && /(seo|meta|search).{0,12}title|title.{0,12}(seo|meta|search)/.test(text)) {
      ...
      report.push({ field: field.slug, label: field.label, status: "transferred", note: "Separate SEO/meta title selected in Destiny. Confirm this collection field is bound to the page template's SEO title setting before publishing." });
