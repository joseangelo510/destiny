...
export function prepareDraftBody(body: WebflowDraftBody) {
  ...
  return { websiteId, articleKey, title, metaTitle, contentHtml, metaDescription, wordCount, graphics };
}
...
/** Remove Rebound SEO-embedded figures so an image-rejecting Webflow still gets the full article text. */
...
export function planWebflowFieldData(input: {
  ...
  const report: FieldReportEntry[] = [
    { field: titleField, label: "Article headline", status: "transferred", note: "Visible article headline." },
    { field: "slug", label: "Slug", status: "transferred", note: input.includeSlug ? "Stable, collision-safe slug generated by Rebound SEO." : "Existing slug kept — updates never change a draft's slug." },
  ...
  for (const field of normalizeFields(input.fields)) {
    ...
    if ((field.type === "PlainText" || field.type === "TextArea") && /(seo|meta|search).{0,12}title|title.{0,12}(seo|meta|search)/.test(text)) {
      fieldData[field.slug] = article.metaTitle;
      report.push({ field: field.slug, label: field.label, status: "transferred", note: "Separate SEO/meta title selected in Rebound SEO. Confirm this collection field is bound to the page template's SEO title setting before publishing." });
      metaTitleMapped = true;
      continue;
    }
    ...
    if (field.type === "Image" && /thumb|main|hero|feature|cover|image|photo/.test(text)) {
      ...
      if (graphic) {
        ...
        imageFieldSlugs.push(field.slug);
        report.push({ field: field.slug, label: field.label, status: "transferred", note: "Rebound SEO original graphic with its alt text." });
        graphicsMapped = true;
      } else {
    ...
    if (field.type === "Number" && /read|min/.test(text)) {
      ...
      if (minutes > 0) {
        ...
        report.push({ field: field.slug, label: field.label, status: "needs_review", note: "Rebound SEO could not calculate a reading time for this article." });
      }
      continue;
    }
    if (field.type === "Option" && /categor|topic|tag|type|section/.test(text)) {
      ...
      if (optionId) {
        ...
        report.push({ field: field.slug, label: field.label, status: "needs_review", note: "Pick a category in Webflow — Rebound SEO only reuses categories already established in this collection." });
      }
      continue;
    }
    ...
    report.push({ field: field.slug, label: field.label, status: "needs_review", note: "Rebound SEO has no matching article content for this field — review it in Webflow." });
  }
  ...
  if (article.metaTitle && !metaTitleMapped) {
    report.push({ field: "", label: "SEO/meta title", status: "unavailable", note: "This collection has no separate SEO/meta-title field; the article headline was transferred as the item title." });
  }
  if ((mainGraphic || extraGraphics.length) && !graphicsMapped) {
    report.push({ field: "", label: "Article graphics", status: "unavailable", note: "This collection has no image field to receive Rebound SEO's graphics." });
  }
...
export function applyImageFallbackToReport(report: FieldReportEntry[], imageFieldSlugs: string[], embeddedGraphicCount: number): FieldReportEntry[] {
  return report.map((entry) => {
    ...
    if (embeddedGraphicCount && entry.label.startsWith("Additional graphics")) {
      return { ...entry, status: "needs_review", note: "Webflow rejected the hosted images — download the remaining Rebound SEO graphics and place them in the article body in Webflow." };
    }
    return entry;
  });
}
...
/**
 * Staged-item endpoints only. Rebound SEO never calls /items/live or any publish
