...
function coachAction(input: RankDigestEmailInput) {
  if (!input.digest.rows.length) return {
    text: `Choose 3 to 5 of the ${input.opportunities.length} opportunities Rebound SEO found. Your next email will become a simple scoreboard: where you rank, what moved, and what to do next.`,
  ...
  return {
    text: best ? `Keep momentum on “${best.keyword}.” Open Rebound SEO to see the page and next recommended action behind this ranking.` : "Open Rebound SEO to see the one ranking action worth doing next.",
    href: input.rankUrl,
...
export function renderRankDigestEmail(input: RankDigestEmailInput) {
  ...
  const firstNotice = input.firstNotice ? `<div class="notice"><strong>New:</strong> Rebound SEO will send this ranking update on your selected schedule. You can change it or turn it off in Account.</div>` : "";
  const visibleOpportunities = input.opportunities.slice(0, 10);
  const opportunitySummary = visibleOpportunities.length > 3 ? `<p class="more">+ ${visibleOpportunities.length - 3} more opportunities ranked inside Rebound SEO</p>` : "";
  const opportunityContent = visibleOpportunities.length
    ...
    : `<div class="opportunity"><strong>No additional opportunities are ready yet.</strong><p>Rebound SEO will add recommendations here after the next audit finds relevant searches with meaningful demand.</p></div>`;
  ...
  const opportunitiesSection = `<section class="opportunities"><div class="section-heading"><div><span>${startingPoint ? `${visibleOpportunities.length} opportunities Rebound SEO found` : "Opportunities Rebound SEO found"}</span><small>Suggestions · not tracked yet</small></div></div><p>Relevant searches that are not in your tracked list. Ranked by business fit, buying intent, attainable difficulty, and meaningful search demand.</p>${opportunityContent}</section>`;
