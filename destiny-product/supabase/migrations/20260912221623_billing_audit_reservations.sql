-- One initial research attempt per verified account, retained after site deletion.
alter table public.billing_accounts add column free_audit_claimed_at timestamptz;
create function public.begin_billed_audit(p_website_id uuid,p_user_id uuid,p_provider text,p_livemode boolean)
...
begin
  ...
  if account.free_audit_claimed_at is null then
    free_grant:=true;
