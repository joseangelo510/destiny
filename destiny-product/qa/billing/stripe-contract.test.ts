...
it("accepts a paid upgrade proration for the current item but rejects credits and unrelated invoices", () => {
  const item = subscription.latest_invoice.lines.data[0];
  const upgraded = { ...subscription, latest_invoice: { ...subscription.latest_invoice, lines: { data: [{ ...item, amount: 5000, parent: { subscription_item_details: { subscription_item: "si_123", proration: true } }, period: { start: 1500, end: 2000 } }] } } };
  const validate = (value: unknown) => validateSubscription(value as Stripe.Subscription, { customerId: "cus_123", livemode: false, priceIds });
  expect(validate(upgraded).paidThrough).toBe(new Date(2000*1000).toISOString());
  for (const invoice of [
    { ...upgraded.latest_invoice, status: "open" },
    { ...upgraded.latest_invoice, customer: "cus_other" },
    { ...upgraded.latest_invoice, parent: { subscription_details: { subscription: "sub_other" } } },
    { ...upgraded.latest_invoice, lines: { data: [{ ...upgraded.latest_invoice.lines.data[0], amount: -5000 }] } },
  ]) expect(validate({ ...upgraded, latest_invoice: invoice }).paidThrough).toBeNull();
});
