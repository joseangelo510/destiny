...
it("settles successful work only after the provider completes", async () => {
  ...
  expect(rpc.mock.calls[1]).toEqual(["finish_billing_usage", { p_id: "usage-a", p_succeeded: true, p_provider_cost_usd: null }]);
});
it("returns the unit on a failed provider response", async () => {
  ...
  expect(rpc.mock.calls[1][1].p_succeeded).toBe(false);
});
