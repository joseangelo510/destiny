...
it("uses the RLS-visible website owner for a server-signed reservation", async () => {
  vi.stubGlobal("Deno", { env: { get: () => secret } });
  ...
  const response = await usage.fetch(new Request("https://example.invalid", { method: "POST", body, headers }), { userClaims: { id: "owner-a" }, supabase: { from: () => ({ select: () => ({ eq: () => ({ maybeSingle: async () => ({ data: { organizations: { owner_id: "owner-a" } }, error: null }) }) }) }) }, supabaseAdmin: { rpc } } as never);
