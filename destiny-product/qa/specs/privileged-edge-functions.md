...
## Billing boundaries (updated September 24, 2026; local implementation)
...
The authenticated billing status read may return an effective-price summary from a Stripe invoice preview. The service selects the customer and subscription only from the verified owner's mirrored billing account, then verifies customer metadata, subscription ownership, live/test mode and USD currency before returning bounded display fields. It exposes no Stripe identifiers. Missing, mismatched or unavailable provider data returns an unavailable estimate without changing billing readiness or access. The page separates the catalog price from the estimated next payment and rejects a preview whose subtotal does not match the mirrored plan. This remains candidate behavior until the Edge and web changes are deployed and read back from the signed-in owner account.

App proxies preserve recognized billing denials through a fixed local mapping. `BILLING_MANAGED_WEBSITE_REQUIRED` produces HTTP 402, an actionable owner-selection message and the local billing path; upstream worker text and supplied URLs are discarded. Account-level Keyword Research remains intentionally unscoped until a keyword is saved, added to strategy or tracked. The managed-site denial applies to paid work that already carries a verified website scope.
