...
export default async function AccountPage() {
  ...
  return <WorkspaceShell active="/account" eyebrow="Account settings" title="Your account" description="See exactly which email is signed in and control where and how often Rebound SEO sends updates."><p><Link className="secondary-button" href="/account/billing">Plans, usage and billing</Link></p><p><Link className="secondary-button" href="/account/password?next=%2Faccount">Set or change your password</Link></p><AccountSettings activeWebsiteId={context.website?.id ?? null} loginEmail={loginEmail} notificationEmail={context.website?.notification_email ?? context.profile?.contact_email ?? null} websites={context.websites.map((website) => {
