...
export default async function AccountPage() {
  ...
  const { data: commsPreference } = context.website
    ...
    : { data: null };
  return <WorkspaceShell active="/account" eyebrow="Account settings" title="Your account" description="See exactly which email is signed in and where Destiny sends your audit updates."><AccountSettings activeWebsiteId={context.website?.id ?? null} initialCommsPreference={commsPreference ? { cadence: commsPreference.cadence, userTimezone: commsPreference.user_timezone, emailEnabled: commsPreference.email_enabled, pushEnabled: commsPreference.push_enabled } : undefined} loginEmail={loginEmail} notificationEmail={context.website?.notification_email ?? context.profile?.contact_email ?? null} websites={context.websites.map((website) => ({ id: website.id, businessName: website.business_name, normalizedDomain: website.normalized_domain }))} /></WorkspaceShell>;
}
