...
export default async function KeywordsPage({ searchParams }: { searchParams?: Promise<{ discover?: string }> }) {
  const discoveryRound = Math.max(0, Math.min(5, Math.floor(Number((await searchParams)?.discover) || 0)));
  ...
  const newResearch = await loadNewKeywordRecommendations(context, Math.floor(discoveryRound));
