...
export default async function ReoptimizationDocumentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <WorkspaceShell active="/keywords" eyebrow="Re-optimization document" title={`Improve “${manifest.keyword}”`} description="Copy-ready changes in page order. Destiny never publishes these edits without your review.">
    ...
    <section className={styles.nextStep}><h2>4. Your next step</h2><p>Begin with Section 1, then complete the heading table in page order. Review every claim before changing the CMS; Destiny does not publish automatically.</p></section>
  </WorkspaceShell>;
}
