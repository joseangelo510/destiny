...
export default async function KeywordResearchPage({ searchParams }: { searchParams: Promise<{ from?: string }> }) {
  ...
  const { supabase, website, audit } = await getWorkspaceContext();
  const [{ data: listRows }, { data: savedRows }] = website ? await Promise.all([
    supabase.from("rank_tracker_lists").select("id,name").eq("website_id", website.id).order("created_at"),
    supabase.from("tracked_keywords").select("keyword,list_id").eq("website_id", website.id).eq("status", "paused").eq("source", "research").order("created_at"),
  ]) : [{ data: [] }, { data: [] }];
  ...
  >
    ...
    <KeywordResearchWorkspace
      auditId={audit?.id ?? ""}
      initialLists={(listRows ?? []).map((list) => ({ id: list.id, name: list.name }))}
      initialQuery={website?.normalized_domain ?? ""}
      initialSavedKeywords={(savedRows ?? []).map((row) => ({ keyword: row.keyword, listId: row.list_id }))}
