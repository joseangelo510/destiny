...
export async function invokeBillingWorker(client: SupabaseClient, endpoint: "billing-usage" | "seo-research", payload: Record<string, unknown>) {
...
export async function reserveContentWork(client: SupabaseClient, websiteId: string, meter: "articles" | "shortOutputs" | "infographics" | "audits", requestKey: string): Promise<{ id: string; response?: never } | { id?: never; response: Response }> {
...
export async function finishContentWork(client: SupabaseClient, id: string, succeeded: boolean): Promise<boolean> {
  try {
