...
export async function loadBillingAccount(ownerId: string) {
  ...
  if (error) return { available: false as const, checkoutReady: false, mode: null, account: null, access: billingAccess(null), used: {} as Record<string, number> };
  ...
  if (since && access.canRunPaidWork) {
    ...
    if (usageError) return { available: false as const, checkoutReady: false, mode: null, account: data, access, used };
