...
export async function runDestinyLogic(
  ...
  return runDestinyLogicFromBytes(input, bytes);
}

export async function runDestinyLogicFromBytes(
  input: DestinyLogicInput,
  bytes: BufferSource,
): Promise<DestinyLogicResult> {
  const output: string[] = [];
  ...
  const imports = {
    env: {
      ...
      parse_int: (handle: number) =>
        BigInt(Number.parseInt(readText(handle).trim(), 10) || 0),
      args: () => {
