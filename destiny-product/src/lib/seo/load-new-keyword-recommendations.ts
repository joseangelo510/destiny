import { loadSufficientDiscovery, retainDiscoveryOrder } from "./keyword-discovery-recovery";
import "server-only";
...
export async function loadNewKeywordRecommendations(context: Awaited<ReturnType<typeof getWorkspaceContext>>, round = 0) {
  ...
  try {
    ...
    }, currentRound), (pool) => applyKeywordPreferenceSignals(pool.keywords, signals).filter(keyword => !signals.some(signal => signal.normalizedKeyword === keyword.keyword.toLowerCase() && signal.decision === "approved")).length, Math.max(0, Math.min(5, Math.floor(round))));
    return { ...result, keywords: retainDiscoveryOrder(result.keywords, applyKeywordPreferenceSignals(result.keywords, signals)) };
  } catch {
    return { keywords: [], status: "unavailable" as const, round };
  }
}
