...
export function InterviewsWorkspace({
  ...
  async function startInterview(topic: InterviewTopic) {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "Rebound SEO could not start this interview.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function startCustomInterview() {
    ...
    if (!title) {
      setError("Add the topic you want Rebound SEO to interview you about.");
      return;
    }
  ...
  async function decideInsight(item: VoiceLibraryItemView, decision: "confirmed_by_owner" | "rejected_by_owner") {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(parseError(payload, "Rebound SEO could not save that decision."));
    } catch (cause) {
      setItems(previous);
  ...
  async function createArticle() {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok || typeof payload.contentUrl !== "string") throw new Error(parseError(payload, "Rebound SEO could not prepare the article draft."));
      window.location.assign(payload.contentUrl);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Rebound SEO could not prepare the article draft.");
      setBusy(false);
    }
  }
  ...
  return <div className={styles.workspace}>
    <section className={styles.promise}>
      ...
      <p>Rebound SEO turns the conversation into an article in your own words—and every interview makes future content sound more like you and less like generic AI.</p>
    </section>
    ...
    {stage === "topics" && <section className={styles.view}>
      <div className={styles.viewHeading}><span>Rebound SEO has the questions</span><h2>Ready to be interviewed?</h2><p>Choose a topic based on your approved keywords and business context. Rebound SEO asks one clear question at a time.</p></div>
      <div className={styles.trustNote}><span>🔒</span><p><strong>Your answers are saved word-for-word.</strong> Your first interview will start your Voice Library, and Rebound SEO will never invent your experience.</p></div>
