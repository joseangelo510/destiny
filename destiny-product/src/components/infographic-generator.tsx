...
export function InfographicGenerator({ websiteId, websiteName, approvedKeywords, generationAvailable }: {
  ...
  async function downloadArticle() {
    ...
    try {
      const blob = await downloadResponse(await fetch("/api/content/infographic/document", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ websiteId, plan }) }), "Rebound SEO could not create the document.");
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Rebound SEO could not create the document."); }
    finally { setBusy(""); }
  }
  ...
  return <div className="infographic-generator">
    <section className="infographic-intro workspace-card">
      <div><span className="feature-eyebrow">INFOGRAPHIC GENERATOR</span><h2>Turn one useful topic into a visual story.</h2><p>Rebound SEO researches current evidence first. You review the facts before an image is created.</p></div>
    ...
    <section className="infographic-deliverables" aria-label="What Rebound SEO creates">
    ...
    <section className="workspace-card infographic-setup">
      ...
      <div className="infographic-form-grid">
        ...
        <label className="infographic-full"><span>Special instructions <small>Optional</small></span><textarea rows={3} value={specialInstructions} onChange={(event) => setSpecialInstructions(event.target.value)} placeholder="Mention an audience, industry, or point of view Rebound SEO should consider." /></label>
      </div>
