...
const AI_DESTINATIONS = [
  ...
  { name: "Replit", mark: "R", description: "Connect the project repository so Rebound SEO can propose approved content changes." },
  ...
  { name: "ChatGPT / Claude-built website", mark: "AI", description: "The AI created the site; Rebound SEO connects to where its files or content are managed." },
] as const;
...
export function PublishingDestinations({
  ...
  return <section className="publishing-destinations" id="publishing-destinations">
    <div className="workspace-card-heading integration-section-heading"><div><strong>Publishing destinations</strong><small>Choose where Rebound SEO should place content after you review and approve it. Rebound SEO will create drafts—not publish without you.</small></div></div>

    <div className="publishing-destination-featured integration-row">
      <span className="integration-logo">W</span>
      <div><strong>WordPress</strong><p>Verify a revocable Application Password. Draft transfer is the first live publishing workflow Rebound SEO is completing.</p><p className="integration-summary">{wordpressConnected ? `${wordpressDisplayName || "WordPress editor"} · ${wordpressSiteUrl || "Verified site"}` : "Connect your WordPress site securely."}</p></div>
      <span className={`status-chip ${wordpressConnected ? "" : "amber"}`}>{wordpressConnected ? "Verified" : "Available now"}</span>
      <div className="publishing-destination-actions">
    ...
    <div className="publishing-destination-featured integration-row">
      <span className="integration-logo">W</span>
      <div><strong>Webflow</strong><p>Verify a site API token with CMS access, then choose the collection where Rebound SEO creates draft items—never published items.</p><p className="integration-summary">{webflowConnected ? webflowSummary || "Verified Webflow CMS collection" : "Connect your Webflow site securely."}</p></div>
    ...
    <div className="publishing-destination-group">
      <div className="publishing-destination-group-heading"><strong>CMS and website platforms</strong><small>Popular destinations included in the Rebound SEO connection roadmap.</small></div>
    ...
    <div className="publishing-destination-group">
      <div className="publishing-destination-group-heading"><strong>AI-built and code-based websites</strong><small>Rebound SEO connects to the builder, repository, or CMS that actually controls the live website.</small></div>
