...
export function DraftDashboard({ view }: { view: ReboundDraftView }) {
  ...
  const publicationLabel = publication?.state === "verified_live" ? "Verified live" : publication?.state === "published_needs_review" ? "Published in WordPress — needs review" : publication ? "Published in WordPress — checking" : null;
  ...
  return <Shell active="/app/content" draftActions subtitle="review and approve the saved draft" title="Content" view={view}><div className={styles.page}>
    ...
    <NeedsYouBar detail={publication?.reason ?? (view.publicationCheckFailed ? "WordPress status is unavailable; Rebound SEO cannot safely offer scheduling yet." : view.draft.approved ? "Reopen only if this article needs another editing pass." : "Approval is available here only after the existing article-quality checks pass.")} move={{ href: "#draft-actions", label: view.draft.approved ? "Review approval" : "Check approval" }} title={publicationLabel ?? (view.draft.approved ? "This saved draft is approved" : "Review this saved draft")} />
    <div className={styles.articleLayout}><article className={styles.document}><div className={styles.kicker}><StateChip state={status} /><span>{view.draft.keyword}</span><span>{view.draft.generationStatus.replaceAll("_", " ")}</span><span>Updated {updated}</span></div><h2>{view.draft.title}</h2><p className={styles.meta}>Exact saved article body. Approval uses the same quality rules as Content Studio.</p><div className={styles.articleBody}>{view.draft.body}</div></article><aside className={styles.articleRail}><div className={styles.railAction}><span>YOUR MOVE</span><h3>{publication ? "This article is already published in WordPress." : view.draft.approved ? "Approved and ready for its next delivery step." : "Approve when the article passes every saved quality check."}</h3><p>{publication?.reason ?? "Edit the draft in Content Studio when revisions are needed. Rebound SEO does not record a separate edit request here."}</p><DraftApprovalActions auditId={view.auditId} draft={view.draft.data} draftId={view.draft.id} publicationUnavailable={view.publicationCheckFailed} publishedInWordPress={Boolean(publication)} websiteId={view.websiteId} /></div><Panel><PanelHeader title="Current saved state" /><div className={styles.railRows}><p><span>Keyword</span><b>{view.draft.keyword}</b></p><p><span>Draft</span><StateChip state={status} /></p><p><span>Generation</span><b>{view.draft.generationStatus.replaceAll("_", " ")}</b></p><p><span>Verification</span>{publicationLabel ? <b>{publicationLabel}</b> : <EvidenceChip fallback="After publish" />}</p></div></Panel></aside></div>
  </div></Shell>;
}
...
export function CalendarDashboard({ view, selectedKeyword }: { view: ReboundCalendarView; selectedKeyword?: string }) {
  ...
  return <Shell active="/app/calendar" calendarActions view={view}><div className={styles.page}>
    ...
    <CalendarPlanner key={view.websiteId} calendar={data.calendar} approvedDrafts={view.approvedDrafts} studioHref={studioHref} timeZone={view.planTimezone} websiteId={view.websiteId} selectedKeyword={selectedKeyword} />
...
export function DistributionDashboard({ view }: { view: ReboundDistributionView }) {
  ...
  return <Shell active="/app/distribution" distributionActions view={view}><div className={styles.page}>
    ...
    <p className={styles.read}><b>Your voice, your move:</b> Review the saved context, then join the conversation as yourself. You decide what gets posted.</p>
  </div></Shell>;
}
...
export function ProgressDashboard({ view }: { view: ReboundProgressView }) {
  ...
  return <Shell active="/app/progress" progressActions view={view}><div className={styles.page}>
    ...
    <section><div className={styles.sectionHeading}><h2>What’s been done</h2><span>your completed moves and the evidence behind them</span></div><Panel><div className={styles.doneList}>{data.done.map((item) => <article key={item.id}><time>{item.at ? new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" }).format(new Date(item.at)) : "Saved"}</time><div><strong>{item.title}</strong><small>{item.detail}</small></div><EvidenceChip evidence={{ kind: item.evidenceKind, source: item.evidenceKind === "verified" ? "quest" : "quest", observedAt: item.at, detail: item.evidenceKind === "verified" ? "Verified" : "Reported" }} /><Action href={item.href} label={item.moveLabel} websiteId={view.websiteId} /></article>)}</div></Panel></section>
