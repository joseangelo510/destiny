  ...
  return <main className={styles.stage} data-rebound-core="v1">
    <div className={styles.canvas}>
      ...
      <section className={styles.main}>
        <header className={styles.topbar}><div><h1>{title ?? current?.label ?? "Rebound SEO"}</h1><span>{subtitle ?? PAGE_SUBTITLES[active] ?? "read-only workspace"}</span></div><WebsiteSwitcher active={active} className={styles.mobileSiteContext} websiteId={websiteId} websites={websites} /><EvidenceChip fallback={searchConnected ? "Search Console connected" : "Search Console not connected"} /></header>
