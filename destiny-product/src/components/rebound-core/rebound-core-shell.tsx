...
export function ReboundCoreShell({ active, calendarActions = false, children, distributionActions = false, draftActions = false, queue, websiteId, websiteLabel, searchConnected, title, subtitle }: { active: string; calendarActions?: boolean; children: ReactNode; distributionActions?: boolean; draftActions?: boolean; queue: PanelResult<CoreQueue>; websiteId: string; websiteLabel: string; searchConnected: boolean; title?: string; subtitle?: string }) {
  ...
  return <main className={styles.stage} data-rebound-core="v1">
    <div className={styles.canvas}>
      ...
      <section className={styles.main}>
        ...
        <PreviewStrip calendarActions={calendarActions} distributionActions={distributionActions} draftActions={draftActions} />
        {children}
