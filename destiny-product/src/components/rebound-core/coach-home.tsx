...
function DoneDefinition({ move }: { move: CoreMove }) {
  ...
  return <section className={styles.donebox} aria-label="What done looks like">
    ...
    <ul>
      <li><i aria-hidden="true" /><div>{move.state === "draft" ? "Read the draft." : keywordReview ? "Review the suggested keywords." : "Follow the recommended steps."}<small>You</small></div></li>
      <li><i aria-hidden="true" /><div>{move.state === "draft" ? "Approve or request edits." : keywordReview ? "Approve, decline or re-optimize." : "Save your work in the tool."}<small>{move.state === "draft" ? "You" : "You · save your decision to complete this step"}</small></div></li>
...
function StatusStrip({ view }: { view: ReboundHomeView }) {
  ...
  return <section className={styles.ticker} aria-label="Current workspace status" tabIndex={0}>
    ...
    {drafts > 0 ? <span><b>{drafts} draft{drafts === 1 ? "" : "s"}</b> awaiting review</span> : null}
