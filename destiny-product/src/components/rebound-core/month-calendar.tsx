...
export function MonthCalendar({ data, emptyDayHref }: { data: CalendarSummary; emptyDayHref?: string }) {
  ...
  return <>
    ...
    <div className={styles.calendarGrid} data-empty-month={emptyMonth && !emptyDayHref}><div>Mon</div><div>Tue</div><div>Wed</div><div>Thu</div><div>Fri</div><div>Sat</div><div>Sun</div>{cells.map((cell) => <section className={cell.inMonth ? styles.calendarDay : `${styles.calendarDay} ${styles.outsideMonth}`} key={cell.key}><span>{cell.date.getUTCDate()}</span>{cell.events.slice(0, 2).map((event) => <p className={styles[`event_${event.tone}`]} key={event.id}>{event.title}</p>)}{cell.inMonth && !cell.events.length && emptyDayHref ? <a aria-label={`Add content on ${cell.key}`} className={styles.calendarAdd} href={emptyDayHref}>+ add content</a> : null}</section>)}</div>
    ...
    {data.suggestions?.length ? <section className={styles.calendarSuggestions}><header><strong>Approved keyword topics</strong><span>Ready to plan · not scheduled yet</span></header><div>{data.suggestions.map((suggestion) => <article key={suggestion.id}><span>Suggested</span><strong>{suggestion.title}</strong><small>Approved topic · not scheduled</small></article>)}</div></section> : null}
  </>;
}
