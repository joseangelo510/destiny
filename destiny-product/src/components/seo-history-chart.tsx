import { buildLinePath, formatHistoricalCount, historyMonthLabel, latestHistoryPoints, type HistoricalSeoPoint } from "../lib/analytics/history";
...
export function SeoHistoryChart({
  ...
  return <article className="workspace-card seo-history-chart">
    <div className="history-chart-heading"><div><span className="eyebrow">Last 3 months</span><h2>{title}</h2><p>{description}</p></div><div><strong>{formatHistoricalCount(latest)}</strong><small>{change === null ? "Baseline established" : `${change >= 0 ? "+" : ""}${change}% over the period`}</small></div></div>
