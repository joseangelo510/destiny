...
export function CreatorDiscovery({ initialCreators, paid, topics, websiteId }: {
  ...
  return <>
    <div className="creator-discovery-note"><strong>Audience filter</strong><span>Target 3,000–100,000 followers. Rebound SEO verifies topic relevance first; audience size must be confirmed before outreach.</span></div>
    {paid ? <div className="creator-discovery-actions"><button className="secondary-button" disabled={loading || !topics.length} onClick={() => void discover()} type="button">{loading ? "Finding creators…" : "Get more creator recommendations (5 searches)"}</button><small>{updatedAt ? `Public search checked ${new Date(updatedAt).toLocaleString()}.` : `Researches: ${topics.slice(0, 3).join(" · ") || "Approve priority keywords first"}`}</small></div> : null}
