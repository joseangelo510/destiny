...
## D10.0 — Redesign v1 · Gate 0: policy re-pin + Slice 1 authorization

Date: 2026-08-31

Authority: Fable 5 High (deciding). Executor: Codex.

Classification: HIGH — governance (policy pin resolution). Bundled MEDIUM authorization: Foundation + read-only Home + design docs.

Policy: HARNESS_POLICY.md @ 14cbda0e36fe217892cdfd1e4946c036edfb1e55 (origin/main).
Prior pin 24c0ee825df6ca9359a4dfadf25779b15cef7ece is unreachable from all canonical refs and is VOID.
Policy file sha256: fd2fde61e20d2946c8cd793e82daeeaf5e9be93235b31b41bf79c384da03b34c
Rationale: repository policy outranks pointer; policy is reachable at the working SHA; re-pin recorded here is authoritative.

Scope (MEDIUM, authorized):

- New unlinked authenticated routes: /app/home only in Slice 1 (Slice 2 routes pre-classified MEDIUM, sequenced after Slice 1 acceptance).
- Shell, shared components, typed read-only adapters over existing tables/integration data.
- docs/design/redesign-v1/{PLAN.md, rebound-notes-v5.html, rebound-five-pillars.html, README.md}.
- Text-only update of any in-repo pointer citing 24c0ee8… → 14cbda0….

Allowed paths: the new /app route directory (or the reported nested equivalent); a new shared components directory; a new adapters directory; new tests/fixtures/stories for the above; docs/design/redesign-v1/; DEPLOY_LOG.md (append only); the in-repo governance pointer text (SHA string only).

Disallowed paths: middleware, auth, session, RLS, supabase/migrations, env files, CI/deploy workflows, package manifests except non-sensitive devDependencies, public/, all existing routes and tools.

Out of scope (HOLD; each needs its own decision): feature flag/allowlist; GA4/Cloudflare connect flow; Cloudflare provider; touchpoint/event ledger; cadence/milestone/last_seen_at storage; any write action; cutover/redirects/retirement of old routes; release tags; CI/deploy changes; changes to existing tools.

Gates:

1. Codex records policy sha256 + verbatim classification clauses in DISCOVERY.md; matches discovery summary.
2. /app/* confirmed covered by existing auth + workspace resolution with zero auth/middleware edits (or nested equivalent reported).
3. PR opened on protected branch with required tests, screenshots, git diff --stat; three guards green.
4. Check-in with merge SHA + guard URLs; Fable 5 High returns go/hold for Slice 2.

Stop conditions: §6 items 1–10 of D10.0 decision text.

Rollback: revert the PR; delete the new route/components/adapters/docs directories; no data, config, or auth state is touched, so no further reversal exists.

Claim boundary: completion means "Slice 1 merged with evidence" — not cutover, not availability to users, not a release. No user-facing announcement. Old routes and tools unchanged.
