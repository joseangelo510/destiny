...
## D10.30 - Owner-authorized release-access diagnosis and recovery

- Recorded before implementation: September 11, 2026 UTC. HIGH: narrowly scoped CI diagnosis/recovery. Sole Codex task01a08dd5-6cf8-7a51-9849-0c30234bd5f5; active task list verified no other owner. Canonical base b26f9c6590667c383df96a0b2cb9620bcfdcbe0b, branch codex/rebound-release-access-diagnostic. Existing approved release PR124/head609b297197f1966015f0d389d6403791ff0de617 unchanged.
- Owner request verbatim: "Figure out a way to unblock and process. Ask Claude fable if needed". Source: direct Jose message in this task after the three-turn API403 blocked audit. Continues D10.28/D10.29 scoped shipping authorization and permits necessary diagnosis/recovery, with Claude/Fable advisory review when useful. Fable5.1 Extra is asked to review the diagnostic, not to grant owner approval.
- Confirmed evidence: complete local/CI and staging green for PR124; required policy/checklist statuses pending because evaluators fail first GET /pulls with403. Jobs explicitly have PullRequests:read and Statuses:write. Repeated unchanged runs fail, latest checklist103179296054 at07:09:40UTC. Body/headers discarded, so permission versus rate limit is unresolved. All-open-PR reconciliation amplifies requests; no assumed diagnosis.
- Diagnostic scope: one new push-only workflow restricted exactly to this diagnostic branch, minimum contents:read/pull-requests:read, no status/package/deployment/environment writes; reviewed Node script makes only GET /rate_limit and GET repository open pulls with per_page=1. Print only status, allowlisted safe response headers, numeric core quota and sanitized error message/documentation URL. Never raw responses, credentials, request headers or production secrets. Mock tests cover exact endpoints/GET-only, token redaction, primary/secondary403 states and errors. Separate record, RED test and GREEN implementation commits.
- No product, configuration, authentication, schema, permission escalation, protected policy, branch rule, release pin or data change. No new PAT/app/token. This standalone read-only diagnostic branch is not a release source and does not publish required status; retain its immutable run/log evidence. Inspect all push triggers first to avoid CI-triggered API bursts. A persistent repair, if needed, must use protected PR/full harness/exact-head review and preserve all approval/stale-head controls; never bypass them.
- Recovery: follow confirmed response reset/retry timing, then invoke existing unchanged guards once each with PR124 first; merge only with current-head checks green and existing OEA. Existing release tag/deploy authorization remains restricted to reviewed product and wrapper. Production stays v1.1.9 until guarded deployment; rollback unchanged. No uncontrolled retries or production writes during diagnosis.
