...
## D10.42 - Repair service-role Auth verification for metered product work

- Recorded before implementation: September 24, 2026 UTC. HIGH because this changes database functions at an Auth/security boundary through an additive migration and coordinated Edge callers. Base `c3a51270ca85cebd711a5c5d4539063131abb0df`, canonical `joseangelo510/destiny`, clean branch `codex/audit-billing-permission`. Sole implementation owner is Codex task `01a0c2b9-ffe5-70a1-b974-2daef7dba3fd`; the active task inventory was rechecked and found no other active Destiny implementation owner.
- Owner requests verbatim: “You're supposed to do it for me.” and “Be more aggressive to reach the goal. you keep waiting for my permsision. you control the goal”. Source: Jose's active Rebound SEO acceptance goal and direct follow-up messages in this task. Scope is limited to preparing and validating the confirmed service-role permission repair required for the requested real-user product acceptance. Protected merge, migration application, Edge deployment, release tag, production changes, provider funding and customer CMS publication remain separate actions.
- Confirmed production evidence: `public.begin_billed_audit`, `public.reserve_competitor_suggestions` and `public.reserve_transactional_email` are `SECURITY INVOKER`, executable only by `service_role`, and each directly queries `auth.users.email_confirmed_at`. Production `service_role` has Auth schema usage but no `SELECT` privilege on `auth.users`; `postgres` has that privilege. The observed audit call therefore fails before reservation or audit creation, and the same privilege defect is structurally present in onboarding suggestions and transactional email reservation. Do not grant `service_role` broad access to the managed Auth table and do not convert the RPCs to `SECURITY DEFINER` merely to bypass the failure.
- Plan: add versioned service-only `SECURITY INVOKER` RPCs that accept explicit verified-identity flags from trusted Edge callers, reject false or missing verification, and preserve the existing owner/member checks, locks, idempotency, allowances, site scoping and billing records. Edge callers will establish verification with the existing server-only Auth Admin `getUserById` pattern before calling the versioned RPCs. Keep the old RPCs during coordinated rollout so the migration can land before Edge code without breaking the currently deployed bundle. No user metadata, browser-supplied verification, new grants on `auth.users`, policy weakening, secrets/configuration, pricing, data deletion or production mutation is allowed.
- Evidence required: separate RED and GREEN commits; unit tests for verified, unverified and unavailable Auth lookups; disposable-database tests executed as `service_role` proving the versioned RPCs work without `auth.users` privileges, still deny browser roles and preserve cross-site/quota/concurrency behavior; full `pnpm gate`; exact-head Codex review; staging build identity and changed-route zero-5xx evidence; policy/checklist guards. Rollback reverts Edge callers to the prior immutable bundle and leaves the additive versioned functions unused until a protected cleanup migration. Live acceptance requires the authorized migration and Edge release followed by one real ClearCheck audit attempt; a prepared or merged candidate is not live proof.
