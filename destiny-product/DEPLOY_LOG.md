...
## D10.43 - Show the authenticated account's effective Stripe payment estimate

- Recorded before implementation: September 24, 2026 UTC. HIGH because this reads and renders authenticated customer financial state through the billing Edge boundary, although it creates no payment, discount, subscription, schema or entitlement change. Base `c3a51270ca85cebd711a5c5d4539063131abb0df`, canonical `joseangelo510/destiny`, clean branch `codex/effective-billing-display`. Sole implementation owner is Codex task `01a0c2b9-ffe5-70a1-b974-2daef7dba3fd`; the active worktree and open-PR inventory were checked before implementation.
- Owner request source: the active Rebound acceptance goal says pricing features have blocked demos, asks whether the owner's account can be free, and directs Codex to test every feature and fix what is not working. Follow-up instructions say “You're supposed to do it for me.” and “Be more aggressive to reach the goal. you keep waiting for my permsision. you control the goal”. This authorizes preparation and validation of the display repair. Protected merge, Edge/web deployment, release tags, payment actions, subscription changes, discount changes and production mutation remain separate actions.
- Confirmed behavior: the owner account is active Premium with a Stripe-verified 100% `Complimentary owner access` discount and an estimated $0 next payment, while the authenticated Rebound billing page shows only the $250/month catalog plan price. The page currently reads the mirrored plan/status and a billing configuration readiness response; it does not request an effective amount or discount from Stripe. Do not calculate coupon arithmetic in the browser, hard-code the owner email or discount, or weaken normal plan and usage enforcement.
- Plan: extend the existing authenticated billing `status` response with a bounded, server-derived next-invoice preview for the exact mirrored Stripe customer/subscription. Verify customer ownership, subscription/customer identity, livemode and currency; return only sanitized amount, date and discount display fields. Treat preview failure as unavailable presentation while preserving billing readiness and existing portal access. Render catalog price separately from the estimated next payment, discount duration and next date. Keep public pricing unchanged and never expose customer/subscription/coupon IDs or the portal session URL.
- Evidence required: separate RED and GREEN commits; unit tests for verified ownership, wrong customer/mode, absent subscription, preview failure, percent/amount discounts and bounded sanitized output; page rendering tests for $0 complimentary and unavailable-summary states; full `pnpm gate`; exact-head Codex review; hosted harness, staging identity and billing-page zero-5xx evidence; policy/checklist guards. Rollback reverts the response and rendering together through a protected change. Live acceptance requires a signed-in ClearCheck billing readback showing the actual effective estimate; a candidate is not live proof.

## D10.44 - Preserve managed-website recovery through app proxies

- Recorded before implementation: September 24, 2026 UTC. HIGH because this changes authenticated billing-denial presentation shared by paid-work routes, although it changes no plan, quota, selection, entitlement, schema or provider operation. Continue on branch `codex/effective-billing-display` at reviewed parent `d8f5ddb03a9c04a7d2f2dbcc203d7f19f1a49634`; sole owner remains Codex task `01a0c2b9-ffe5-70a1-b974-2daef7dba3fd`.
- Live/source evidence: site-scoped workers already return HTTP 402 with `BILLING_MANAGED_WEBSITE_REQUIRED` and truthful selection guidance. `billingFailureResponse` accepts only four other billing codes, so app routes fall back to the Supabase client message `Edge Function returned a non-2xx status code`. A September 24 Jose Angelo Studios research attempt reconfirmed the generic copy and no visible usage increase; that Keyword Research request itself is account-metered, so it is provider-failure evidence rather than a managed-site denial. The source defect applies when a website-scoped paid-work route receives the managed-site code.
- Plan: add the existing managed-site code to the shared sanitized billing-failure map with the established owner-selection message. Preserve HTTP 402 and the local `/account/billing` recovery URL so existing clients show their current site-scoped billing link. Add a failing-first regression proving upstream text and untrusted billing URLs remain hidden. Do not change account-level Keyword Research behavior in this repair.
- Evidence required: RED/GREEN commits, focused failure-response test, full exact-head gate and technical review, hosted harness/staging and final signed-in reproduction after release. Rollback reverts the single mapping entry with the test. Candidate state is not live proof.
