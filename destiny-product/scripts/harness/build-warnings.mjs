...
export function evaluateBuildWarnings(rawOutput, policy, now = new Date()) {
  ...
  const knownFingerprints = policy.warnings.map((warning) => warning.fingerprint).filter(Boolean);
  const unknownWarnings = output.split("\n")
    .map((line) => line.trim())
    .filter((line) => WARNING_LINE.test(line))
    .filter((line) => !knownFingerprints.some((fingerprint) => line.includes(fingerprint)));
