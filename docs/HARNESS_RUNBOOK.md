...
A maintainer may explicitly dispatch either existing guard workflow with a PR number to refresh a stuck check. Manual dispatch executes the selected workflow revision (GitHub `github.sha`) so a reviewed maintenance candidate can be tested before first activation; always verify the selected repository, branch and full SHA. Automatic events never check out PR code with write permission. No code from artifacts is executed. No ruleset or notification setting changes are part of D10.22.
