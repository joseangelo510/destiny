...
The current locked floors are 91% changed-line coverage, 89% changed-branch coverage, 69% changed mutation, 100% API contract coverage, 65.52% browser journey coverage, and 87.18% combined route proof. Duplication may not exceed 3.01%. `qa/harness/baseline.v2.json` is authoritative and records each ratchet movement with its receipt.
